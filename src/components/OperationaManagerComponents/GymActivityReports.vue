        ...
        >
          <template v-slot:bottom>
            ...
            <strong
              ...
              {{
                classActivity
                  .map((item) => parseInt(item.JUMLAH_MEMBER))
                  .reduce((a, b) => a + b, 0)
              }}</strong
            >
          </template>
...
export default defineComponent({
  name: 'GymActivityReportsComponent',
  setup() {
    const $q = useQuasar();
    ...
    const columns = [
      {
        ...
        label: 'Date',
        align: 'left',
        field: (row) => row.TANGGAL_BOOKING_GYM,
        format: (val) =>
          `${
            [
              'Sunday',
